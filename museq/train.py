import pybam
import numpy
#import scipy
import features
import Nfeatures
import sys
import argparse
#import pylab
import matplotlib
matplotlib.use("Agg")
import matplotlib.pyplot as plt
import pickle

from sklearn.ensemble import RandomForestClassifier
from sklearn import cross_validation
from sklearn.metrics import roc_curve, auc, confusion_matrix
from sklearn import tree

def saveObject(obj, filename):
    with open(filename, 'wb') as output:
        pickle.dump(obj, output, pickle.HIGHEST_PROTOCOL)
        
def extract_data(files_name):
    data={}	
    for case in files_name:
        print >> sys.stderr, case
        reference = None
        normal = None
        tumour = None

        c = (float(30), float(30), float(70), float(0))

        for line in case:
            l = line.strip().split()
            if len(l) < 3:
                continue

            if line[0] == '#':
                if l[1] == "tumour":
                    tumour = l[2]
                elif l[1] == "normal":
                    normal = l[2]
                elif l[1] == "reference":
                    reference = l[2]
                elif l[1] == "contamination":
                    c = (float(l[2]), float(l[3]), float(l[4]), float(1))
                continue
       
            if not reference:
                continue
        
            if reference not in data:
                data[reference] = {}
            chromosome = l[0]
            position = l[1]
            if chromosome not in data[reference]:
                data[reference][chromosome] = {}
            if (normal, tumour) not in data[reference][chromosome]:
                data[reference][chromosome][(normal, tumour)] = []

            if l[2] == args.label:
                label = 1
            else:
                label = -1

            data[reference][chromosome][(normal, tumour)].append((int(position), label, c))
    return data
            
def extract_features(data):	
    features = []
    labels = []
    keys = []

    for reference in data:
	ref = pybam.Fasta(reference)
        for chromosome in data[reference]:
            ref.load(chromosome)
            for normal, tumour in data[reference][chromosome]:
                print normal
                n_bam = pybam.Bam(normal)
                print tumour
                t_bam = pybam.Bam(tumour)
                if t_bam == None:
                    continue
                if n_bam == None:
                    continue

                for (position, label, c) in data[reference][chromosome][(normal, tumour)]:
                    tumour_data = None
                    for x in t_bam.vector(chromosome, position, position + 1):
                        tumour_data = x
                        break
                    if not tumour_data:
                        continue

                    normal_data = None
                    for x in n_bam.vector(chromosome, position, position + 1):
                        normal_data = x
                        break
                    if not normal_data:
                        continue

                    ref_data = ref.vector(position)

                    _f = []
                    for _, feature in feature_set:
                        _f.append(feature(tumour_data, normal_data, ref_data))
                    for _, feature in coverage_features:
                        _f.append(feature(tumour_data, normal_data, c))
                    t_counts = (tumour_data[1][1], tumour_data[2][1], tumour_data[3][1],
                            tumour_data[4][1], tumour_data[5][1])
                    n_counts = (normal_data[1][1], normal_data[2][1], normal_data[3][1],
                            normal_data[4][1], normal_data[5][1])
                    _f.append(n_bam.xentropy(n_counts, t_counts))
                    features.append(_f)
                    labels.append(label)
                    keys.append((reference, normal, tumour, chromosome, position, label))
    keys=numpy.array(keys)
    features = numpy.array(features)
    labels = numpy.array(labels)
    return features, labels, keys
		                
mutationSeq_version="3.0.0"
bases = ('A', 'C', 'G', 'T', 'N')


parser = argparse.ArgumentParser(description='''train a model''')

parser.add_argument("input_files", metavar='FILE', nargs='*', default=[sys.stdin],
                    type=argparse.FileType('r'), help= "A list of space delimited samples: chromosom position label, the file has a header of normal, tumour and reference file names")

#parser.add_argument("--scale", action="store_true", default=False, help="scale feature vector")
parser.add_argument("--version", default=None, help="Version of the MutationSeq")
parser.add_argument("--normalized", default=False, action="store_true", help="If you want to train with normalized features")
parser.add_argument("--C", default=False, action="store_true", help="If you want to test on deep data you need to change containation rate")
parser.add_argument("--out", default=None, help="save output to file")
parser.add_argument("--validate", metavar='FILE', nargs='*', type=argparse.FileType('r'), default=None, help="To validat the same format file with known lable")
parser.add_argument("--label", default="SOMATIC",help="Label in training file list")

args = parser.parse_args()
if not args.normalized:
    feature_set = features.feature_set
    coverage_features = features.coverage_features
    extra_features = (("xentropy", 0), ("SENTINEL", 0))
    version = features.version
    c = (float(30), float(30), float(70), float(0))
    if args.C:
        c = (float(100000), float(100000), float(70), float(0))
        print "Warning: you are trying to test deep data with unnormalized features"
else:
    feature_set = Nfeatures.feature_set
    coverage_features = Nfeatures.coverage_features
    extra_features = (("xentropy", 0), ("SENTINEL", 0))
    version = Nfeatures.version
    c = (float(30), float(30), float(70), float(0))
    if args.C:
        c = (float(10000), float(10000), float(70), float(0))
        
if args.version:
    print "Version: ", mutationSeq_version
    exit(1)  
        
        

data=extract_data(args.input_files)
feature, labels, keys=extract_features(data)
numpy.savez(args.out, version, feature, labels)

model = RandomForestClassifier(random_state=0, n_estimators=3000, n_jobs=-1, compute_importances=True)
model.fit(feature, labels)
saveObject(model, "model2.npz")
#features_file=open("features_name.txt", 'w')
#for _feature in feature_set + coverage_features + extra_features:
    #print >> _feature[0]
    
#features_file.close()


importance_file = open(args.out + "_importance.txt", 'w')

#for importance, _feature in (zip(model.feature_importances_, feature_set + coverage_features + extra_features)):
#    print >> importance_file, 
for importance, _feature in sorted(zip(model.feature_importances_,
                                       feature_set + coverage_features + extra_features)):
    print >> importance_file, _feature[0], importance
importance_file.close()

if args.validate:
    validation=extract_data(args.validate)
    val_features, val_labels, val_keys=extract_features(validation)
    probs = model.predict_proba(val_features)
    voted = probs[:,1]
    fpr, tpr, thresholds = roc_curve(val_labels, voted)
    roc_auc = auc(fpr, tpr)
    print roc_auc
    fd=open(args.out+'_result', 'w')
    for f,k in zip(voted,val_keys):
        print >> fd, k[0]+' '+k[1]+' '+k[2]+' '+k[3]+' '+k[4]+' '+k[5]+' '+str(f)
    fd.close()  
    plt.plot(fpr,tpr,'k--', label='ROC curve (area = %0.3f)' %float(roc_auc))
    plt.title('ROC curve (area = %0.3f)' %float(roc_auc))
    plt.xlabel('False Positive Rate')
    plt.ylabel('True Positive Rate')
    plt.savefig(args.out + "_roc.png")
    #pylab.plot(fpr, tpr, 'k--', label='ROC curve (area = %0.3f)' %float(roc_auc))
    #pylab.plot([0, 1], [0, 1], 'k--')
    #pylab.xlim([0.0, 1.0])
    #pylab.ylim([0.0, 1.0])
    #pylab.xlabel('False Positive Rate')
    #pylab.ylabel('True Positive Rate')
    #pylab.savefig(args.out + "_roc.png")

else:
    cv = cross_validation.StratifiedKFold(labels, n_folds=3)
    for i, (train, test) in enumerate(cv):
        _model = model.fit(feature[train], labels[train])

        probs = _model.predict_proba(feature[test])
        voted = probs[:,1]
        fpr, tpr, thresholds = roc_curve(labels[test], voted)
        roc_auc = auc(fpr, tpr)
        fd=open(args.out+'_result_'+str(i), 'w')
        for f,k in zip(voted,keys[test]):
            print >> fd, k[0]+' '+k[1]+' '+k[2]+' '+k[3]+' '+k[4]+' '+k[5]+' '+str(f)
        fd.close()
        plt.plot(fpr, tpr, 'k--', lw=1, label="Fold %i (AUC=%0.3f)" % (i + 1, float(roc_auc)))

    
    plt.legend(loc="lower right", numpoints=1,)
    plt.savefig(args.out + "_rocxval.png")

scores = []
candidates = model.estimators_
for _ in candidates:
    scores.append(0)

for f, label in zip(feature, labels):
    for estimator, index in zip(candidates, range(len(candidates))):
        if estimator.predict(f)[0] == label:
            scores[index] += 1 

winner = candidates[numpy.argmax(numpy.array(scores))]
tree.export_graphviz(winner, out_file=open(args.out + "_tree.dot", 'w'),)
