...
pybam_BamIter_next(pybam_BamIter *self) {
    ...
    while (((self->position >= self->buffer->end) || (self->buffer->size == 0)) &&
        ...
        //self->pileup->iter->flag_mask &= 772; // disable the BAM_FDUP
        if(self->deep_flag){ // to check for deepseq data, J
...
Bam_counts(pybam_BamObject *self, PyObject *args) {
    ...
    stop = 0;
    df = 0;
    if (!PyArg_ParseTuple(args, "si|ii", &s, &df, &start, &stop)) return NULL;
...
Bam_single(pybam_BamObject *self, PyObject *args) {
    char *s;
    uint32_t position;
    if(!PyArg_ParseTuple(args, "si", &s, &position)) {
        return NULL;
    }
...
pileup_func(uint32_t tid, uint32_t pos, int n,
    ...
    uint32_t base_counts[4] = {0, 0, 0, 0};
    column_t column;
    for (i = 0; i < 5; i++) {
        for (j = 0; j < 6; j++) {
...
//FD: added to debug and then commented out    
//     double tt= (double)column.features[4][3];
//     printf("init depth: %f\n", tt);
    ...
    for (r = 0; r < n; r++) {
        ...
        length = alignment.b->core.l_qseq;
//         double tmp=(double)length; 
//         printf("length: %f\n", tmp);
        //ops = b->core.n_cigar;
        alignment = pl[r];
        b = alignment.b;
...
//         double tmp= (double)column.features[4][0];
//         printf("depth: %f\n", tmp);
...
/*     double tmp2=(double)column.features[4][0];
     printf("depth_final: %f\n", tmp2)*/;
    column.major = 0;
    uint16_t max = 0;
    for (i = 0; i < 4; i++) {
...
entropy(uint32_t bases[4], uint32_t depth) {
    uint8_t i;
    double e = 0;
    double pr;
//     float pr;
...
//         pr = (float)bases[i] / (float)depth;
        pr = (double)bases[i] / (double)depth;
//FD: added to debug and then commented out
//         printf("probability: %f\n", pr);
//         printf("probability: %f\n", bases[i]);
//         double temp=(double)depth;
//         printf("probability _depth: %f\n", temp);
        if (pr != 0) {
            e += (log(pr) * pr); 
        }
    }
...
//     uint16_t n_depth, t_depth;
    uint32_t n_depth, t_depth;
    double e;
    float pr_n, pr_t;

    if (!PyArg_ParseTuple(args, "OO", &n_tuple, &t_tuple)) return NULL;
   
    n_depth = (uint32_t)PyInt_AsLong(PyTuple_GET_ITEM(n_tuple, 4));
    t_depth = (uint32_t)PyInt_AsLong(PyTuple_GET_ITEM(t_tuple, 4));
// FD: change to resolve overflow bug for deep museq
